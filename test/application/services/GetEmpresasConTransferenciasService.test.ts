...
describe('GetEmpresasConTransferenciasService', () => {
  it('debe devolver las empresas que hicieron transferencias en un rango de fechas', async () => {
    ...
    const mockTransferenciaRepo: TransferenciaRepository = {
      getTransferenciasPorRango: jest.fn().mockResolvedValue([transferencia1, transferencia2, transferencia3]),
    };
    ...
    const service = new GetEmpresasConTransferenciasService(mockTransferenciaRepo, mockEmpresaRepo);
    ...
    expect(mockTransferenciaRepo.getTransferenciasPorRango).toHaveBeenCalledWith(desde, hasta);
    expect(mockEmpresaRepo.getEmpresasPorIds).toHaveBeenCalledWith(['id1', 'id2']);
  });
});
